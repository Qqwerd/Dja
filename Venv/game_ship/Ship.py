...
# Запуск может занять до 20  сек
# Если происходит бесконечная расстановка то перезапустите
        ...
        if input_type == "shot":
            ...
            if self.is_ai:
                ...
                if x not in Game.letters or not y.isdigit() or int(y) not in range(1, Game.field_size + 1):
                    self.message.append('Не выходите за пределы карты')
                    return 500, 0
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            if shot_result == 'miss':
                ...
                game.next_player.message.append('Плохие новости, корабль  уничтожен :(')
                continue
